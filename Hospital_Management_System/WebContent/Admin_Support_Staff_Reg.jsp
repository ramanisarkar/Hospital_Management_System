...
$(document).ready(function(){
	...
	var username="supporatStaffList";
		$.get('SupportStaff',{flag:username},function(response) {
			var obj = JSON.parse(response);
			$.each(obj, function(index, value) {
			count++;
		   	var html = '';
		   	html += '<tr id='+count+'>';
		   	html += '<td><img src="Support_Staff_Image/'+obj[index].profileimage+'" height="50px" width="50px" class="img-circle"></td>';
			...
			html += '<td><button style="margin-top: 10px;" type="button" class="btn btn-info  edit" data-edit_id="'+count+'" id="supportStaffid'+count+'" value="'+obj[index].id+'">Edit</button>';
			html += '<button style="margin-left: 10px; margin-top: 10px;" type="button" class="btn btn-danger delete" data-delete_id="'+count+'" id="supportStaffId'+count+'" value="'+obj[index].id+'">Delete</button></td></tr>';
			$('.supportstaff').append(html);
	 	});
			 $('#example').DataTable();
	});
	$("#txt_username").keyup(function(){
		
	      var supportingusername = $(this).val().trim();
	      var username = "chackusername";
	      console.log(supportingusername);
	      if(supportingusername != ''){
	         $.ajax({
	            url: 'SupportStaff',
	            type: 'post',
	            data: {flag:username , supportstaffusername1:supportingusername},
	...
	$('#insert_form').on('submit' , function(event){
	    ...
	    $.ajax({
	        ...
	        success: function(response ,textStatus , jqXHR ){
	        	console.log(response);
	        	var obj = JSON.parse(response);
	        	if(obj[0].supportstaffeupdate == "true"){
	        		$('#message1').show();
	        		$('#message2').show();
	        		$('#message3').show();
	        	...
	        	else if(obj[0].supportstaffeemail == "false"){
	        		$('#message1').show();
	        		$('#message2').show();
	        		$('#message3').show();
	        	...
	        	else if(obj[0].supportstaffemailinvalid == "false"){
	        		$('#message1').show();
	        		$('#message2').show();
	        		$('#message3').show();
	...
	$(document).on('click', '.edit', function(){
		...
		console.log(editid);
		var supportStaff = $('#supportStaffid'+editid).val();
		var username = "editSupportStaff";
		$.get('SupportStaff',{flag:username , supportStaffId:supportStaff},function(response){
 			$("#supportStaffTab1").hide();
 			var obj = JSON.parse(response);
			console.log(response);
			$("#supportStaffTab2").show();
			$('.nav-tabs a[href="#menu2"]').tab('show');
			$('#supportstaffid').val(obj[0].id);
			...
			if(obj[0].profileimagename){
				...
				$("#coveredit").attr("src", "Support_Staff_Image/"+obj[0].profileimage+"");
			}else{
				$('#profileimageid').hide();
			}
		});
	...
	$(document).on('click', '.delete', function(){
		...
		var supportStaff = $('#supportStaffId'+deleteid).val();
		var username = "deleteSupportStaff";
		$.get('SupportStaff',{flag:username , supportStaffId:supportStaff},function(response){
 	...
 	$(".nav-tabs a").click(function(){
		...
		$('#message3').hide();
   	    ...
   	    if(supportStaffList=="Support Staff List"){
   	    	location.reload();
   	    }
   	});
});
			...
			<ul class="nav nav-tabs tabcalss">
				<li class="active" id="tab1doctorlist"><a href="#home"
					data-toggle="tab" ><i class="fa fa-bars"
						aria-hidden="true" style="border-radius: 50%; padding: 8px;"></i>Support Staff List</a></li>
						
				<li id="tabdoctoradd"style="margin-left: 15px; background-color: f1f4f9;">
				<a href="#menu1" id="supportStaffTab1" data-toggle="tab">
				<i class="fa fa-plus-circle"aria-hidden="true" style="border-radius: 50%; padding: 8px;"></i>Add New Support Staff</a>
				</li>
				<li id="tabdoctoradd"style="margin-left: 15px; background-color: f1f4f9;"><a 
				id="supportStaffTab2" data-toggle="tab"
					href="#menu2" style="display: none;"><i class="fas fa-edit"
						aria-hidden="true" style="border-radius: 50%; padding: 8px;"></i>Edit Support Staff</a>
				</li>
			</ul>
			        ...
			        <thead>
			            <tr>
			                <th>Photo</th>
							 ...
							 <th style="padding-left: 15px;"> Doctor Email</th>
							 <th style="padding-left: 15px;">Action</th>
			            </tr>
			        </thead>
			        <tbody class="supportstaff">
				...
				<div style="margin-top: 10px;">
				    ...
				    <div class="form-group row">
			    	 ...
			    	 <div class="form-group row">
			    		...
			    		<input type="hidden" id="supportstaffid" name="supportstaffid" >
